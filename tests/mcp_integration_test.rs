...
async fn test_network_connectivity_tools(client: &mantra_dex_sdk::client::MantraDexClient) {
    ...
    match config_result {
        ...
        Err(Error::Contract(msg)) => {
            println!("    ⚠ Pool manager config test - expected mock error: {}", msg);
        }
...
async fn test_wallet_balance_tools(client: &mantra_dex_sdk::client::MantraDexClient) {
    ...
    match balances_result {
        ...
        Err(Error::Wallet(msg)) => {
            println!("    ⚠ Wallet balances test - expected wallet error: {}", msg);
        }
        Err(Error::Contract(msg)) => {
...
async fn test_pool_query_tools(client: &mantra_dex_sdk::client::MantraDexClient) {
    ...
    match pools_result {
        Ok(pools) => {
            println!("    ✓ Pool information retrieved: {} pools found", pools.len());
            if !pools.is_empty() {
    ...
    match specific_pool_result {
        ...
        Err(Error::Contract(msg)) => {
            println!("    ⚠ Specific pool query test - expected mock error: {}", msg);
        }
...
async fn test_dex_core_trading_tools(client: &mantra_dex_sdk::client::MantraDexClient) {
    ...
    let provide_liquidity_result = client
        .provide_liquidity(pool_id, assets.clone(), liquidity_max_slippage, swap_max_slippage)
        .await;
    ...
    match provide_liquidity_result {
        ...
        Err(Error::Contract(msg)) => {
            println!("    ⚠ Provide liquidity test - expected mock error: {}", msg);
        }
    ...
    let withdraw_result = client
        .withdraw_liquidity(pool_id, lp_token_amount)
        .await;
    ...
    match withdraw_result {
        ...
        Err(Error::Contract(msg)) => {
            println!("    ⚠ Withdraw liquidity test - expected mock error: {}", msg);
        }
    ...
    let pool_type = mantra_dex_std::pool_manager::PoolType::StableSwap {
        amp: 100u64,
    };
    
    let create_pool_result = client
        .create_pool(asset_denoms, asset_decimals, pool_fees, pool_type, Some("new_integration_pool".to_string()))
        .await;
    ...
    match create_pool_result {
        ...
        Err(Error::Contract(msg)) => {
            println!("    ⚠ Create pool test - expected mock error (admin only): {}", msg);
        }
        Err(Error::FeeValidation(msg)) => {
            println!("    ⚠ Create pool test - expected fee validation error (admin only): {}", msg);
        }
...
async fn test_lp_token_management_tools(client: &mantra_dex_sdk::client::MantraDexClient) {
    ...
    match balance_result {
        Ok(balances) => {
            println!("    ✓ Balances retrieved (including LP tokens): {} coins", balances.len());
            // Filter for LP tokens (typically have format factory/{pool_contract}/lp)
            let lp_tokens: Vec<_> = balances.iter()
...
async fn test_mcp_tools_error_handling() {
    ...
    match zero_swap_result {
        ...
        Err(e) => {
            println!("  ✓ Error handling works for zero amount (different error type): {:?}", e);
        }
    }
...
async fn test_mcp_tools_performance() {
    ...
    for operation in operations {
        ...
        match result {
            ...
            Err(_) => {
                panic!("Operation {} timed out after {:?}", operation, operation_timeout);
            }
        }
    }
