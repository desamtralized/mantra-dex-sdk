...
impl CliConfig {
    ...
    pub fn add_wallet(&mut self, name: &str, mnemonic: &str, password: &str) -> Result<(), CliError> {
        if self.wallets.contains_key(name) {
            return Err(CliError::Wallet(format!("Wallet '{}' already exists", name)));
        }
    ...
    pub fn get_wallet_mnemonic(&self, name: &str, password: &str) -> Result<String, CliError> {
        let encrypted = self.wallets.get(name)
