...
pub fn load_contract_addresses(network: &str) -> ContractAddresses {
    let config_dir = std::env::var("MANTRA_CONFIG_DIR").unwrap_or_else(|_| "config".to_string());
...
pub async fn get_om_usdc_pool_id(client: &MantraDexClient) -> Option<String> {
    ...
    for pool in pools {
        if pool.pool_info.assets.iter().any(|a| a.denom == uom_denom) && pool.pool_info.assets.iter().any(|a| a.denom == uusdc_denom) {
