    ...
    pub async fn execute(self, mut config: CliConfig) -> Result<(), CliError> {
        ...
        match self.command {
            WalletCommands::Create { name } => {
                ...
                let network_constants = load_network_constants(&config)?;
                let (wallet, mnemonic) =
                    MantraWallet::generate(&network_constants).map_err(CliError::Sdk)?;
                ...
                {
                    return Err(CliError::Command(
                        "Wallet creation cancelled - please save your mnemonic before continuing"
                            .to_string(),
                    ));
                }
                ...
                print_success(&format!(
                    "Wallet '{}' created, encrypted, and set as active",
                    name
                ));

                // Show wallet info
                let address = wallet.address().map_err(CliError::Sdk)?.to_string();
            ...
            WalletCommands::Import { name, mnemonic } => {
                ...
                print_success(&format!(
                    "Wallet '{}' imported, encrypted, and set as active",
                    name
                ));
            ...
            WalletCommands::List => {
                ...
                for (name, _) in &config.wallets {
                    let address = if let Some(active) = config.active_wallet.as_ref() {
                        if active == name {
                            ...
                            match unlock_wallet(&config, name) {
                                Ok((wallet, _)) => {
                                    wallet.address().map_err(CliError::Sdk)?.to_string()
                                }
                    ...
                    rows.push(vec![active, name.to_string(), address]);
                }
            ...
            WalletCommands::Export { name } => {
                ...
                if config.get_session_password().is_none() {
                    ...
                    let password = Password::new()
                        .with_prompt(&format!(
                            "Enter the same password again to store in session"
                        ))
                        .interact()
                        .map_err(|e| CliError::Command(format!("Input error: {}", e)))?;
