...
impl ProvideLiquidityCommand {
    pub async fn execute(self, config: CliConfig) -> Result<(), CliError> {
        ...
        if !self.yes {
            println!(
                "\nYou are about to provide liquidity to pool {}:",
                self.pool_id
            );
            for asset in &assets {
                println!("- {}", asset);
...
impl WithdrawLiquidityCommand {
    pub async fn execute(self, config: CliConfig) -> Result<(), CliError> {
        ...
        if !self.yes {
            println!(
                "\nYou are about to withdraw {} shares from pool {}.",
                amount, self.pool_id
            );
        ...
        print_success(&format!(
            "Successfully withdrawn liquidity from pool {}",
            self.pool_id
        ));
