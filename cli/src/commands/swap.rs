    ...
    pub async fn execute(self, config: CliConfig) -> Result<(), CliError> {
        ...
        if self.simulate {
            ...
            if !output_amount.is_zero() {
                ...
                println!("\nEffective Price: {} {} per {}", price_ratio, offer_asset.denom, self.ask_denom);
                
                let inverse_price = Decimal::from_ratio(output_amount, input_amount);
                println!("Inverse Price: {} {} per {}", inverse_price, self.ask_denom, offer_asset.denom);
            }
            ...
            let result = client
                .swap(&self.pool_id, offer_asset.clone(), &self.ask_denom, Some(max_spread))
                .await
                .map_err(CliError::Sdk)?;
